...
test('方向键导航：ArrowRight 入右子节点、ArrowLeft 回根', async ({ page }) => {
  ...
  await expect(page.getByText('方向键图').first()).toBeVisible()

  // 两个子节点（smoke/collapse 同款录入链路：点根选中 → Tab 建子 → 等编辑框 → 输入 → 点空白提交）
  for (const name of ['分支甲', '分支乙']) {
    await page.getByText('方向键图').first().click()
  ...
  await page.getByText('方向键图').first().click()
  await expect(page.locator('g.active')).toContainText('方向键图')

  // ArrowRight：选中移到右侧子节点之一。两子节点与根等距时引擎取 bfsWalk 首见者
  // （checkNodeDis 严格 <，见核验 (a)），故断言「移到两子节点之一」而非具体哪个
  await page.keyboard.press('ArrowRight')
  await expect(page.locator('g.active')).not.toContainText('方向键图')
...
test('编辑框内方向键：移动光标而非跳节点', async ({ page }) => {
  ...
  await page.getByTestId('btn-confirm').click()
  await expect(page.getByText('编辑框方向键').first()).toBeVisible()

  // 一个子节点：若框内 ArrowRight 被劫持，选中会跳到它
  await page.getByText('编辑框方向键').first().click()
  await page.keyboard.press('Tab')
  await expect(page.locator('div.smm-node-edit-wrap')).toBeVisible()
  ...
  await page.getByText('编辑框方向键').first().dblclick()
  ...
  await expect(editor).toBeVisible()
  await expect(page.locator('g.active')).toContainText('编辑框方向键')
