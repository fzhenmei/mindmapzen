...
// 开屏流程（M5d spec §2）：?nows=1 跳过 harness 预设工作区 → 无工作区首启态 →
// 开屏页主按钮「创建工作区」→ pickDirectory 桩返回 /ws → 真实 setWorkspace 链路 → 案头出现
test('开屏：无工作区首启 → 创建工作区 → 进入案头', async ({ page }) => {
  ...
  // 主按钮「创建工作区」= pickDirectory 流（harness 桩固定返回 /ws）→ 案头出现（空态引导）
