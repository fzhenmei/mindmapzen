...
// 2026-09-06 备注合并后 copyIncludeNote 退役：下拉两项（保留双链/含正文）。
test('砚栏复制选项：默认值、切换、重开保持与配置落盘', async ({ page }) => {
  ...
  // 展开下拉：两项（保留双链=on、含正文=on）——「包含备注」已随 copyIncludeNote 退役
  ...
  await expect(page.getByTestId('copy-include-body')).not.toBeChecked()
  await page.keyboard.press('Escape')
  // 配置文件落盘断言（内存 fs）：仅两键，退役的 copyIncludeNote 不复活
  const cfg = await page.evaluate(() =>
    (window as unknown as { __zenE2e: { readFile(p: string): Promise<string> } }).__zenE2e.readFile('/cfg.json'),
  )
  expect(JSON.parse(cfg).settings).toEqual({ copyIncludeLinks: false, copyIncludeBody: false })
})
