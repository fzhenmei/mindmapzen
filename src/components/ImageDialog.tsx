...
export default function ImageDialog({ nodeText, current, preview, pasteError, onPick, onPaste, onPasteClick, onRemove, onCancel }: Readonly<Props>) {
  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onCancel() }}>
      <DialogContent data-testid="image-dialog" aria-label={t('editor.imageDialog.title')} className="sm:max-w-xl" onPaste={handlePaste}>
        ...
        >
          {preview !== null ? (
            ...
            <span className="text-sm text-destructive" data-testid="image-missing">
              {t('editor.imageDialog.missing', { path: current?.src })}
            </span>
          ) : (
            <span className="flex flex-col items-center gap-2 text-sm text-muted-foreground">
              <IconImage size={32} />
              {t('editor.imageDialog.empty')}
            </span>
          )}
        </div>
        {pasteError !== null && (
