...
export async function rgbaToPngBytes(width: number, height: number, rgba: Uint8Array): Promise<Uint8Array> {
  ...
  if (ctx === null) throw new Error('无法创建 2D 画布上下文')
  ...
  const blob = await new Promise<Blob>((resolve, reject) => canvas.toBlob((b) => (b === null ? reject(new Error('PNG 编码失败')) : resolve(b)), 'image/png'))
